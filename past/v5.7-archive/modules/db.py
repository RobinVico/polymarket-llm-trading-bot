...
def save_position_meta(token_id, market_slug, side, entry_price, tp, end_date, initial_size, notes="", entry_reason="", claude_raw_estimate=None, original_confidence=None, stop_loss_tier=None):
    ...
    conn.execute("""INSERT OR REPLACE INTO position_meta
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'pending')""",
        (token_id, market_slug, side, entry_price, tp, target_gap, end_date, initial_size, _utc_now_iso(), notes, entry_reason, claude_raw_estimate, original_confidence, stop_loss_tier))
    conn.commit(); conn.close()
...
def save_closed_position(token_id, market_slug, side, avg_entry, exit_price, size,
    ...
    No 仓位 PnL: (avg - exit) × size (price 下跌 = 赚, No 押注的就是下跌).
    Yes 仓位 PnL: (exit - avg) × size."""
    if side and side.upper() in ("YES",):
        pnl_usd = (exit_price - avg_entry) * size
    else:
        pnl_usd = (avg_entry - exit_price) * size
