...
# 5 次错密码 → 30 分钟 lockout (in-memory, 重启清零, 个人项目够用).
LOGIN_LOCKOUT_MAX = 5
LOGIN_LOCKOUT_WINDOW = 1800  # 30 min
_login_attempts = {}  # ip -> (fail_count, window_start_ts)
...
def _rate_fail(ip):
    rec = _login_attempts.get(ip)
    now = time.time()
    if not rec or now - rec[1] > LOGIN_LOCKOUT_WINDOW:
...
def create_app():
    ...
    def login_route():
        ...
        if flask_request.method == "POST":
            ...
            if expected and pw == expected:
                ...
                return redirect("/")
            _rate_fail(ip)
            log.warning(f"login fail: ip={ip} attempts={_login_attempts.get(ip,(0,0))[0]}")
    ...
    def index():
        ...
        cash = exe.get_cash_balance() or 0  # None (API 失败) 退化为 0, 只为本次渲染不挂
    ...
    def execute_state():
        ...
        from modules.db import get_position_meta, log_event, mark_executed_action, update_monitor_state, clear_position_meta
        try:
            data = flask_request.get_json() or {}
            token_id = data.get("token_id", "")
    ...
    def snapshot():
        ...
        try:
            ...
            total_cost = 0.0
            total_value = 0.0
            cash = exe.get_cash_balance() or 0  # None (API 失败) 退化为 0
