...
from modules.monitor import TIME_STOP_DAYS, TIME_STOP_DRIFT_PP, HOLD_MIN_EDGE_PP, SOFT_NEGATIVE_THRESHOLD_PP, DISASTER_DROP_PP
...
.ms-CONFIRMED_NEGATIVE{background:rgba(255,64,112,0.2);color:#ff4070;animation:reevalPulse 1.5s ease-in-out infinite}
.ms-AT_TARGET{background:rgba(0,229,160,0.2);color:#00a884;animation:reevalPulse 2s ease-in-out infinite}
.ms-BLACKSWAN_HEDGE{background:rgba(128,96,255,0.2);color:#8060ff}
...
{% if st in ("CONFIRMED_NEGATIVE", "BLACKSWAN_HEDGE") %}
<span class="ms-badge ms-{{ st }} ms-clickable" onclick="executeState('{{ p.asset }}','{{ st }}','{{ p.title|replace("'","&apos;")|truncate(50) }}', {{ p.size }}, {{ p.cur_price }})">{{ st }} <span style="font-size:9px;opacity:0.7">点击执行</span></span>
{% elif st == "AT_TARGET" %}
...
<div class="rule"><span class="k">SOFT_NEGATIVE</span><span class="v" style="color:#cc9900">edge &lt; -{{ soft_neg_pp_abs }}pp (1次重评后) 警戒</span></div>
<div class="rule"><span class="k">CONFIRMED_NEGATIVE</span><span class="v red">edge &lt; -{{ soft_neg_pp_abs }}pp (2次重评后) 建议清仓</span></div>
<div class="rule"><span class="k">AT_TARGET</span><span class="v" style="color:#00a884">p ≥ q 达目标 建议清仓</span></div>
<div class="rule"><span class="k">BLACKSWAN_HEDGE</span><span class="v" style="color:#8060ff">p ≥ 97% 且距结算 > 1天 → 减半</span></div>
<div class="rule"><span class="k">DISASTER (自动)</span><span class="v red">价跌 -{{ disaster_drop_pp }}pp → 自动整笔卖</span></div>
...
function reevalExit(tokenId, title, size, curPrice){
  // 跟 executeState 一样, 但用 CONFIRMED_NEGATIVE 走清仓路径
  // 我们直接调 execute_state api, 让后端绕过 state mismatch 校验
  // 简化: 用户既然选了 exit, 就直接 sell
  ...
  // 直接调 execute_state 但要先把 monitor_state 改成 CONFIRMED_NEGATIVE 否则后端会拒
  // 用一个新的 force_exit api 更干净, 但暂时复用 execute_state 加 force 标志
...
function executeState(tokenId, state, title, size, curPrice){
  let action_text = "";
  let sell_size = size;
  if(state === "BLACKSWAN_HEDGE"){
    sell_size = (size * 0.5).toFixed(2);
    action_text = "卖出 50% (约 " + sell_size + " 股) 防黑天鹅";
  } else if(state === "CONFIRMED_NEGATIVE"){
    action_text = "整笔清仓 (" + size + " 股) - thesis 已翻负";
  } else if(state === "AT_TARGET"){
...
def create_app():
    ...
    def execute_state():
        ...
        try:
            ...
            # 决定卖多少
            if confirmed_state == "BLACKSWAN_HEDGE":
                sell_size = round(size * 0.5, 2)
                action_tag = "blackswan_hedged"
                reason = f"BLACKSWAN_HEDGE: 卖 50% ({sell_size} 股)"
            elif confirmed_state in ("CONFIRMED_NEGATIVE", "AT_TARGET"):
                sell_size = size
                action_tag = "user_exited_" + confirmed_state.lower()
                reason = f"{confirmed_state}: 用户确认整笔清仓 ({size} 股)"
            else:
