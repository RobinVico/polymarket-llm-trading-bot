...
def save_position_meta(token_id, market_slug, side, entry_price, tp, end_date, initial_size, notes="", entry_reason="", claude_raw_estimate=None, original_confidence=None):
    ...
    conn.execute("""INSERT OR REPLACE INTO position_meta
        (token_id, market_slug, side, entry_price, tp, target_gap, end_date, initial_size, created_at, notes, entry_reason, claude_raw_estimate, original_confidence, reeval_status)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?, 'pending')""",
        (token_id, market_slug, side, entry_price, tp, target_gap, end_date, initial_size, datetime.now().isoformat(), notes, entry_reason, claude_raw_estimate, original_confidence))
    conn.commit(); conn.close()
...
def get_portfolio_history(since_ts, max_points=500):
    """读 since_ts 之后所有 snapshot. 超过 max_points 自动均匀降采样."""
    conn = get_conn()
    ...
    if len(rows) > max_points:
        ...
        rows = rows[::step][:max_points] + [rows[-1]]
    return rows
